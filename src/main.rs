...
async fn subscribe_pumpfun_creates() {
    ...
    info!(program = PUMPFUN_PROGRAM_ID, "connected, subscribing to pump.fun transactions + block meta");
    ...
    blocks_meta.insert("meta".to_owned(), SubscribeRequestFilterBlocksMeta {});
...
fn flush_stale_pending(pending: &mut HashMap<u64, Vec<CreateEvent>>, latest_slot: u64) {
    ...
    for slot in stale_slots {
        if let Some(evs) = pending.remove(&slot) {
            for ev in evs {
                warn!(slot = ev.slot, "block_time never arrived; emitting without timestamp");
                emit(&ev, None);
            }
        }
    }
